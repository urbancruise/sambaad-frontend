...
export default function EditActivityModal({ open, onClose, activity }: Props) {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 select-none">
            ...
            <div 
                className="absolute inset-0 bg-white/10 backdrop-blur-xs transition-opacity animate-[fadeIn_0.2s_ease-out]" 
                onClick={onClose} 
            />

            {/* Cyberpunk Modal Box Frame */}
            <div className="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-800/90 backdrop-blur-2xl p-8 text-slate-100 shadow-[0_0_60px_rgba(6,182,212,0.15)] animate-[scaleUp_0.25s_ease-out]">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-slate-800/80 mb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-xl font-bold tracking-wide text-black">
                            Modify Activity Parameter Metrics
                        </h2>
                    </div>
                    <button 
                        onClick={onClose}
